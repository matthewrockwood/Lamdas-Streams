...
public class ArraysAndStreams2 {
    public static void main(String[] args) {
        ...
        //to lowercase if a vowel
        System.out.printf("strings that start with a vowel: %s%n",
                Arrays.stream(strings)
                        .map(x -> x.toLowerCase())
                        .filter(x-> x.startsWith("a") || x.startsWith("e") ||
                                x.startsWith("i") || x.startsWith("o") ||
                                x.startsWith("u"))
                        .toList());

        //idk
        System.out.printf("strings concatenated: %s%n",
                Arrays.stream(strings)
                        .collect(Collectors.joining(", ")));

        //part 3
        System.out.printf("Count of strings that contain more than 5 %d%n",
                Arrays.stream(strings)
                        .filter(x -> x.length()> 5)
                        .collect(Collectors.toList())
                        .size()
//found out u can also use .count but whatever
                );
